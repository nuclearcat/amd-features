...
use crate::catalog;
use crate::expectations::{self, Attention, Expectation, ExpectationLevel};
...
pub struct FeatureReport {
    ...
    /// What processors in the recognized hardware class normally or optionally expose.
...
impl Report {
    ...
    fn render_attention_legend(&self, s: &mut String, opts: TextOptions) {
        let has_critical = self
            ...
            .any(|feature| feature.attention == Some(Attention::Critical));
        let has_warning = self
            .categories
            .iter()
            .flat_map(|category| &category.features)
            .any(|feature| feature.attention == Some(Attention::Warning));
        if !has_critical && !has_warning {
            return;
        }
        s.push('\n');
        s.push_str(&bold("Attention", opts.color));
        s.push('\n');
        if has_critical {
            s.push_str(&format!(
                "  {} expected for this hardware class, but absent or disabled\n",
                colorize("! red:", Attention::Critical.color(), opts.color)
            ));
        }
        if has_warning {
            s.push_str(&format!(
                "  {} available on this class, but product/firmware/OS dependent\n",
                colorize("! yellow:", Attention::Warning.color(), opts.color)
            ));
        }
    }
...
fn render_feature(s: &mut String, f: &FeatureReport, opts: TextOptions) {
    let color = f
        .attention
        .map_or_else(|| f.status.color(), Attention::color);
    let glyph = colorize(
        if f.attention.is_some() {
            "!"
        } else {
            f.status.glyph()
        },
    ...
    if let (Some(expectation), Some(attention)) = (f.expectation, f.attention) {
        let qualifier = match expectation.level {
            ...
            ExpectationLevel::Available => "available",
        };
        s.push_str(&colorize(
            &format!(
                "      ! {} for {}: {}\n",
                qualifier, expectation.profile, expectation.rationale
            ),
            attention.color(),
            opts.color,
        ));
