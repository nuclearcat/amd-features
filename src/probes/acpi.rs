...
//! and is itself informative: IVRS ⇒ the platform supports AMD-Vi, NFIT ⇒ persistent
...
const FEATURES: &[&str] = &[
    "amd_vi", "s0ix", "pmem", "cxl", "hmat", "hpet", "numa", "wsmt", "tpm",
];
...
impl Probe for AcpiProbe {
    ...
    fn detect(&self, ctx: &Context) -> ProbeResult {
        ...
        // AMD-Vi: IVRS table = platform support; /sys/class/iommu = active.
        ...
        out.push((
            ...
            if iommu_active {
                Detection::with_detail(
                    ...
                    SRC,
                    if has("IVRS") {
                        "ACPI IVRS + /sys/class/iommu active"
                    } else {
                        "/sys/class/iommu active (IVRS table not visible)"
                    },
                )
            } else if has("IVRS") {
                Detection::with_detail(Status::Present, SRC, "ACPI IVRS table (IOMMU not active)")
            } else if complete {
                Detection::with_detail(Status::Absent, SRC, "no ACPI IVRS table")
            } else {
                Detection::with_detail(
                    Status::Unknown,
                    SRC,
